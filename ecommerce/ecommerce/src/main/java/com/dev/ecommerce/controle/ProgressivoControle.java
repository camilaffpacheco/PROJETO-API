...
public class ProgressivoControle {
	...
	public ModelAndView chamarPercentual(Progressivo progressivo) {
	...
	public ModelAndView salvar(Progressivo progressivo, BindingResult result) {
		...
		if (result.hasErrors()) {
			return chamarPercentual(progressivo);
		}
		finalizarProgressivo(progressivo, result);
		repositorioProgressivo.saveAndFlush(progressivo);
		return chamarPercentual(new Progressivo());
	}
